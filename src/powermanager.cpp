...
PowerManager::PowerManager(QObject *parent)
    : QObject(parent)
    , m_iface("com.yoyo.PowerManager",
              "/CPUManagement", "com.yoyo.CPUManagement",
    ...
    , m_mode(-1)
{
    ...
    QSettings settings(QSettings::UserScope, "yoyoos", "power");
...
void PowerManager::setIdleTime(int idleTime)
{
    if (m_idleTime != idleTime) {
        ...
        QDBusInterface iface("com.yoyo.PowerManager",
                             "/PowerManager", "com.yoyo.PowerManager",
                             QDBusConnection::sessionBus());
        if (iface.isValid()) {
